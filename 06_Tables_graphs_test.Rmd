...
dat_intersex_litt_summ <- dat_intersex_litt_summ %>%
  left_join(dat_tbt_mean %>% 
              ungroup() %>%
              filter(Species %in% "Littorina") %>%
              select(Station, Year, TBT_mean, TBT_flag), by = c("Station", "Year")
            )
...
df_pred_list[["5b"]] <- pred_linear_from_stationname("5", "ISI_mean")
...
# Run 5b using linear regression
df_pred_list[["5b"]] <- pred_linear_from_stationname("5", "Sterile_perc")
