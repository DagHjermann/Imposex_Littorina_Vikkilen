...
#### Modification 9 (11. April 2022)
    - Figur 3: VDSI og TBT bør bytte side slik at VDSI kommer på den primære y-aksen og TBT kommer på den sekundære y-aksen (da blir det samme prinsipp som for Figur 2). *section 7, 'Plot ISI + VDSI, B/W'*   
    - Figur 5: Er det samme Kendall-korrelasjon i Figur 5 etter at 2021-data er inkludert? Hvis ikke, kan jeg få P-verdiene? *section 4, 'Combined plot'*  
    - Figur 6. Vi bytter til T. reticulata slik du skriver nedenfor. Jeg ser at Jonny bruker Tritia reticulatus i NFR-søknaden. Jeg har gjort noen søk, og vi går for reticulata. *section 8, 'Plot biota TBT at all stations (logistic)', result: '06_Timeseries_allsp_TBT_2_log.png'*  
...
library(readxl)
library(stringr)
library(ggrepel)   # geom_text_repel (only used one place, in 8c)
...
# File name and connection
fn <- "Figures/06_Kendall_correlations.txt"
con <- file(fn, open = "w")

# From here, everything is written to file
sink(con)
print_correlations()
sink()
close(con)
...
#### TBT + VDSI, VDSI on right axis (old ) 
```{r}
...
if (save_plots){
  ggsave("Figures/06_Timeseries_Comb_TBT_VDSI_bw_FIG3_OLD.png", gg,
         width = 7, height = 5, dpi = 500)
  ggsave("Figures/06_Timeseries_Comb_TBT_VDSI_bw_FIG3_OLD.svg", gg,
         width = 7, height = 5, dpi = 500)
}
...
#### TBT + VDSI, VDSI on left (not right) axis 
...
gg2 <- gg +
  ggrepel::geom_text_repel(aes(label = paste(Station_name2, Year)), 
                           size = rel(3)) +
...
#### Statistics 1 - use only years/sites with observations of both species    

* Check if there is significant correlation  
...
cor.test(df$Littorina, df$Mytilus)
# p-value = 0.2526

cor.test(df$Littorina, df$Mytilus, method = "kendall")
# p-value = 0.05556

```

#### Statistics 2 - Use all years + sites   
...
# Test plot
if (F)
  ggplot(df, aes(Year, TBT_mean)) +
  geom_point(aes(color = Species2)) + 
  scale_color_brewer(palette = "Dark2") +
  geom_smooth(method = "lm") +
  facet_wrap(vars(Station_name2))

mod1 <- lm(TBT_mean ~ Station_name2*Year, data = df)
mod2 <- lm(TBT_mean ~ Station_name2*Year + Species2, data = df)
